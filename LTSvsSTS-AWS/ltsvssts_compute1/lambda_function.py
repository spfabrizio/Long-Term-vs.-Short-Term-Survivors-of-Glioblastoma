...
# all numeric values from the document for counting absolute number
# of phenotypes per image.
#
...
def phenotype_matrix(s3_client, bucket, filelist, thresholddict, phenotypedict, bucketkey, dbConn):
    count_matrix = {}
...
def lambda_handler(event, context):
  ...
  try:
    print("**STARTING**")
    print("**lambda: ltsvssts_compute1**")
    ...
    if not bucketkey.startswith("LTSvsSTS1-Template/"):
            raise Exception("File is not in 'LTSvsSTS1-Template/' folder. Ignoring event.")
    ...
    df = phenotype_matrix(s3_client, bucketname, filelist, thresholddict, phenotypedict, bucketkey, dbConn)
